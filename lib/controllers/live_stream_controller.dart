...
import 'package:glive/models/app/Rtmp_event.dart';
...
class LiveStreamController extends LiveStreamConnectionManager with LiveStreamControls, LiveStreamTextureMixin {
  LiveStreamController(super.server, super.streamKey) {
    _initialNotifyState();
  }

  @override
  Future<void> dispose() async {
    log("dispose camera live stream controller");
    await _release();
    super.dispose();
  }
...
class LiveStreamConnectionManager with LiveStreamNotifier {
  ...
  Future<void> _createConnection() async {
    ...
    _connectionEventSub?.cancel();
    _connectionEventSub = _connection!.eventChannel.receiveBroadcastStream().listen(_onLiveStreamEvent);
  }
  ...
  Future<void> _createStream() async {
    ...
    _streamEventSub?.cancel();
    _streamEventSub = _stream!.eventChannel.receiveBroadcastStream().listen(_onLiveStreamEvent);
  }

  Future<void> _publishTo(String streamKey) async {
    if (_state.status == LiveStreamStatus.connected || _state.status == LiveStreamStatus.stopped) {
  ...
  void _onConnectionEvent(RtmpConnectionEvent event) async {
    final status = event.code == RtmpConnectionCode.connectSuccess ? LiveStreamStatus.connected : LiveStreamStatus.disconnected;
    _notifyStateChange(status: status);
    _broadcastEvent(event);
  }

  void _onStreamEvent(RtmpStreamEvent event) {
    final status = event.code == RtmpStreamCode.publishStart ? LiveStreamStatus.living : LiveStreamStatus.stopped;
...
mixin LiveStreamNotifier {
  ...
  final StreamController<RtmpEvent> _eventController = StreamController<RtmpEvent>.broadcast();
...
mixin LiveStreamControls on LiveStreamConnectionManager {
  ...
  Future<void> startStreaming() async {
    if (status == LiveStreamStatus.connected || status == LiveStreamStatus.stopped) {
      await _attachAudioAndVideo();
  ...
  Future<void> switchCamera() async {
    final position = _state.cameraPosition == CameraPosition.back ? CameraPosition.front : CameraPosition.back;
...
mixin LiveStreamTextureMixin on LiveStreamConnectionManager {
  ...
  Future<bool> updateTextureSize(Size? size) async {
    ...
    if (size != null && _previousSize != size) {
      ...
      debugPrint("size updated: $size, texture changed: ${textureId != _textureId}");
